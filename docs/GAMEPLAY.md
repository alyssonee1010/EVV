...
- EVVTrapHole (the Trap Digger) is attackable while the digger digs and lays the leaf cover; once covered nobody attacks it and the next Viking to step on it falls in and is gone, together with everyone else standing over the hole or arriving within 0.2 s (a Viking carrying the Girl back too; he drops her). The open hole then fills itself back up for a minute: the tile stays taken and Vikings jump over it in both directions. A digger killed before the cover is done leaves an open hole that fills up the same way.
