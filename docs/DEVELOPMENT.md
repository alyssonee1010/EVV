...
4. Add the collider, renderers, animator, and required animation events.
   If the art has no drawn outline, add EVVSilhouetteOutline to the prefab root so the outline is rendered around the body. List each limb (arm pieces, leg, head) as a group on the component so it keeps its outline where it crosses the body; unlisted sprites form the body. Pieces that lie on the body but should still separate from each other (the two shorts legs) get their own groups marked seamless with body.
